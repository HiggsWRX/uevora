...
forward_check(e(Ninst, Inst), e(NNinst, Inst)) :-
    Inst = [v(n(_),_,V)|_], !,
    remove_valor(Ninst, V, [], NNinst).
forward_check(E, E).

remove_valor([], V, L, L).
remove_valor([v(n(N),D,_)|R], V, E, L) :-
    delete(D, V, ND),
    remove_valor(R, V, [v(n(N),ND,_)|E], L).
remove_valor([v(o(N),D,_)|R], V, E, L) :-
    remove_valor(R, V, [v(o(N),D,_)|E], L).
